...
int Enter_File(FS_Control_Structure *FSCS, char *origin, char *destination) {
  ...
  //open file
  if ((origin_fd = open(origin, O_RDONLY)) == -1) return -2;
  //check size of file
  size_in_bytes = lseek(origin_fd, 0, SEEK_END);
  if (size_in_bytes > MAX_FILE_SIZE) return -9;
  ...
  if (FSCS->unallocated_Data_Blocks_Left < size_in_blocks) return -3;
...
int Extract_File(FS_Control_Structure *FSCS, char *origin, char *destination) {
  ...
  if (file_inode_number == -1) return -1; //file was not found
